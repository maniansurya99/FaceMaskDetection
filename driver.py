        ...
        with torch.no_grad():
            for imgs, labels in loader:
                ...
                if metric_flag:
                    ...
                    df_cm = pd.DataFrame(cm_array, index = [i for i in ['Cloth', 'FFP2', 'n95valve','Surgical',"Without_Mask"]], columns = [i for i in ['Cloth', 'FFP2', 'n95valve','Surgical',"Without_Mask"]])
                    plt.figure(figsize = (10,7))
                    sn.heatmap(df_cm, annot=True)
                    plt.xlabel("Predicted Values")
                    plt.ylabel("Actual Values")
                    plt.savefig('ConfusionMatrix.png')
                    print(metrics.classification_report(predicted, labels, target_names=['Cloth', 'FFP2','n95valve', 'Surgical',"Without_Mask"]))
...
def loadImages(path):
    ...
    image_files += sorted([os.path.join(path, 'n95valve', file)
                           for file in os.listdir(path + "/ffp2")
                           if file.endswith('.jpg')])
...
names = ['Cloth', 'FFP2', 'n95valve','Surgical',"Without_Mask"]
N = []
N.append(len(os.listdir(image_path + "/cloth")))
N.append(len(os.listdir(image_path + "/ffp2")))
N.append(len(os.listdir(image_path + "/n95valve")))
...
def label_gen(d):
    labels = ['Cloth', 'FFP2', 'n95valve', 'Surgical',"Without_Mask"]
    return labels[d]
